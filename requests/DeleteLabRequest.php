 ...
 */
class DeleteLabRequest extends Base
{
	public $lab_id;
	public $user_id;
	private $session_id;
		...
		if($this->valid_session_id($this->session_id, $this->user_id) == 1)
		{
			...
			elseif(count($arrList) == 1)
			{
				...
				if($arrList['owner_user_id'] == $this->user_id)
				{
					...
					{
						$this->errors['lab_deleted'] = "Cannot delete a lab that has already been deleted!"; //derp
					}
					if($arrList['isPublished'] == 1)
					{
						$this->errors['lab_already_published'] = "Cannot delete a lab that has already been published!";
					}
				}
