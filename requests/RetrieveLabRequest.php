 ...
 */
class RetrieveLabRequest extends Base
{
	public $lab_id;
	public $user_id;
	private $session_id;
		...
		if($this->valid_session_id($this->session_id, $this->user_id) == 1)
		{
			...
			elseif(count($arrList) == 1)
			{
				...
				if($arrList['owner_user_id'] != $this->user_id)
				{
					...
					$authorCheck = $this->getDB()->QueryArray('banned_users', 'user_id', "user_id = '{$arrList['owner_user_id']}'");
					
					if($labCheck != false && count($labCheck) > 0)
					{
						$this->errors['lab_deleted'] = "This lab has been deleted!";
					}
					
					if($authorCheck != false && count($authorCheck) > 0)
					{
						$this->errors['banned_author'] = "The author of this lab has been banned!";
					}
				}
