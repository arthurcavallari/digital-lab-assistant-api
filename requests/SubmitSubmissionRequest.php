 ...
 */
class SubmitSubmissionRequest extends Base
{
	public $lab_id;
	public $user_id;
	private $session_id;
	...
	public function validate()
	{
		...
		if($this->valid_session_id($this->session_id, $this->user_id) == 1)
		{
			...
			elseif(count($arrList) == 1)
			{
				...
				$submissionDetails = $this->getDB()->QueryArray('submissions', '*', "laboratory_id = '{$this->lab_id}' AND user_id = '{$this->user_id}'");
				if($submissionDetails == false)
				{
					$this->errors['submission_notfound'] = "A submission for this lab could not be found!";
				}
				elseif(count($submissionDetails) == 1)
				{
					$submissionDetails = $submissionDetails[0];
					$this->submission_id = $submissionDetails['id'];
					if($submissionDetails['isSubmitted'] == 1)
					{
						$this->errors['submission_already_submitted'] = "Cannot submit a submission that has already been submitted!";
					}
				}
			}
		}
